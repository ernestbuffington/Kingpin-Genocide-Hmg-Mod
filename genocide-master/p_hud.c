...
void MoveClientToIntermission (edict_t *ent)
{
	...
	// BEGIN HMG
	ent->client->bonus_timer = 0;
	ent->client->bonus_active = false;
	// END
...
void G_SetStats (edict_t *ent)
{
	...
	*/
	...
	ent->client->ps.stats[STAT_KILLSTREAK] = ent->client->kill_streak;

	if (ent->client->bonus_timer > level.framenum)
		ent->client->ps.stats[STAT_DAMAGETIMER] = (ent->client->bonus_timer - level.framenum)/10;
	else
		{
		ent->client->ps.stats[STAT_DAMAGETIMER] = 0;
		ent->client->HMG_Multiplier = 0;
		ent->client->bonus_active = false;
		}

//	ent->client->ps.stats[STAT_LEADERNAME] = CS_GENERAL + (ent - g_edicts - 1);
