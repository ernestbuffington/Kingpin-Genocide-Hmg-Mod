...
void ED_CallSpawn (edict_t *ent)
{
	...
	// BEGIN: HMG Arena 
	// turn all weapons into HMG's
	if ( (!strcmp(ent->classname, "weapon_pistol")) ||
	     (!strcmp(ent->classname, "weapon_spistol")) ||
		 (!strcmp(ent->classname, "weapon_flamethrower")) ||
		 (!strcmp(ent->classname, "weapon_bazooka")) ||
		 (!strcmp(ent->classname, "weapon_shotgun")) ||
		 (!strcmp(ent->classname, "weapon_grenadelauncher")) ||
		 (!strcmp(ent->classname, "weapon_tommygun")) )
		 {
		 ent->classname = "weapon_heavymachinegun";
		 }

	// If requested remove all ammo except HMG slugs 
	if ( ((!strcmp(ent->classname, "ammo_shells")) ||
		 (!strcmp(ent->classname, "ammo_bullets")) ||
		 (!strcmp(ent->classname, "ammo_cylinder")) ||
		 (!strcmp(ent->classname, "ammo_rockets")) ||
		 (!strcmp(ent->classname, "ammo_flametank")) ||
		 (!strcmp(ent->classname, "ammo_grenades"))) && all_HMG_clips->value)
		{
		G_FreeEdict(ent);
		return;
		}
		...
		{
		// otherwise turn all ammo boxes into HMG slugs
		if ( (!strcmp(ent->classname, "ammo_shells")) ||
			 (!strcmp(ent->classname, "ammo_bullets")) ||
			 (!strcmp(ent->classname, "ammo_cylinder")) ||
			 (!strcmp(ent->classname, "ammo_rockets")) ||
			 (!strcmp(ent->classname, "ammo_flametank")) ||
			 (!strcmp(ent->classname, "ammo_grenades")) )
...
void SP_worldspawn (edict_t *ent)
{
	...
	else
	{
		...
		gi.configstring (CS_GENERAL+(ent - g_edicts - 1), va("%s", "xxxxxxxxx"));
