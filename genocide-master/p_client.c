			...
			if (message)
			{
				...
				if (deathmatch->value)
				{
					if (ff)
					{
						...
						// BEGIN: HMG Arena
						if (attacker->client->bonus_active == false)
							attacker->client->kill_streak++;

						if (attacker->client->kill_streak >= maxstreak->value)
							{
							attacker->client->kill_streak=0;
							...
							if ( PowerUp == 0 )
								{
								attacker->health = 200;
								DisplayBonus0Msg(attacker);
								gi.bprintf (PRINT_HIGH,"%s's Got a Bonus. Watch your back!\n", attacker->client->pers.netname);
								}
							...
							if ( PowerUp == 1 )
								...
								for (i=0 ; i<game.maxclients ; i++)
									...
									// If the player isn't being used or its you continue.
									if ((!cl_ent->inuse) || (cl_ent == attacker))
										continue;
								...
								DisplayBonus1Msg(attacker);
								gi.bprintf (PRINT_HIGH,"%s's Got a Bonus. Watch your back!\n", attacker->client->pers.netname);
								}

							if ( PowerUp == 2 )
								{
								if (attacker->client->bonus_timer > level.framenum)
									{
									diff = attacker->client->bonus_timer - level.framenum;
									attacker->client->bonus_timer += (300 - diff);
									}
								else
									attacker->client->bonus_timer = level.framenum + 300;
								attacker->client->bonus_active = true;
								attacker->client->HMG_Multiplier = 2;
								DisplayBonus2Msg(attacker);
								gi.bprintf (PRINT_HIGH,"%s's Got a Bonus. Watch your back!\n", attacker->client->pers.netname);
								}
							...
							if ( PowerUp == 4 )
								...
								if (attacker->client->bonus_timer > level.framenum)
									...
									attacker->client->bonus_timer = level.framenum + 300;
								attacker->client->bonus_active = true;
								attacker->client->HMG_Multiplier = 3;
								DisplayBonus4Msg(attacker);
								gi.bprintf (PRINT_HIGH,"%s's Got a Bonus. Watch your back!\n", attacker->client->pers.netname);
								}

							if ( PowerUp == 5 )
								{
								attacker->client->pers.pistol_mods |= WEAPON_MOD_COOLING_JACKET;
								attacker->client->pers.hmg_shots = 30;
								DisplayBonus5Msg(attacker);
								gi.bprintf (PRINT_HIGH,"%s's Got a Bonus. Watch your back!\n", attacker->client->pers.netname);
								}
...
//	self->solid = SOLID_NOT;
	...
	// BEGIN HMG
	self->client->bonus_timer = 0;
	self->client->bonus_active = false;
	// END
