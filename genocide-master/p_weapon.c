...
void Weapon_Shotgun (edict_t *ent)
{
	...
	else if (!ent->client->pers.weapon_clip[ent->client->clip_index] && ent->client->weaponstate != WEAPON_DROPPING	 && !ent->client->buttons & BUTTON_ATTACK)
	{
		ent->client->reload_weapon = true;
		ent->client->weaponstate = WEAPON_RELOAD_CYCLE;
		ent->client->ps.gunframe = 17;
...
void weapon_barmachinegun_fire (edict_t *ent)
{
	...
	{	
		...
		if ((ent->client->bonus_timer > level.framenum) && (ent->client->HMG_Multiplier != 0))
			damage = 25 * ent->client->HMG_Multiplier;
		else
			damage = 25;
    ...
    Eject_Tommy_Shell(ent);
	ent->client->gun_noise = true;
/*	ent->client->kill_streak++;

						if (ent->client->kill_streak >= maxstreak->value)
							{
							...
							if ( PowerUp == 1 )
								...
								for (i=0 ; i<game.maxclients ; i++)
									...
									if ((!cl_ent->inuse) || (cl_ent == ent))
										continue;

									cl_ent->health = 10;
	
									}
								DisplayBonus1Msg(ent);
								}

							if ( PowerUp == 2 )
								{
								if (ent->client->bonus_timer > level.framenum)
									{
									diff = ent->client->bonus_timer - level.framenum;
									ent->client->bonus_timer += (300 - diff);
									}
								else
									ent->client->bonus_timer = level.framenum + 300;
								DisplayBonus2Msg(ent);
								}
							...
							if ( PowerUp == 3 )
								...
								ent->client->pers.inventory[ITEM_INDEX(itemhh)] = 100;
								ent->client->pers.inventory[ITEM_INDEX(itemjh)] = 100;
								ent->client->pers.inventory[ITEM_INDEX(itemlh)] = 100;

								DisplayBonus3Msg(ent);
								}
...
void barmachinegun_fire (edict_t *ent)
	...
	else if (ent->client->ps.gunframe == 12)	// speed faster fire, 19 = normal
		{
		if (ent->client->pers.pistol_mods & WEAPON_MOD_COOLING_JACKET)
			{
			ent->client->ps.gunframe = 43;
