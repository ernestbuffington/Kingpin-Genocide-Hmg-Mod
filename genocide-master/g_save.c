...
void InitGame (void)
{
	...
	// BEGIN HMG
	all_HMG_clips = gi.cvar ("HMGclips", "1", CVAR_SERVERINFO | CVAR_LATCH);
	maxstreak = gi.cvar ("maxstreak", "5", CVAR_SERVERINFO | CVAR_LATCH);
	// Lets make sure the streak doesn't get abused.
	if (maxstreak->value > 9)
		maxstreak->value = 9;

	if (maxstreak->value < 3)
		maxstreak->value = 3;

//	DHMG_Start = gi.cvar ("doublefrag", "15", CVAR_SERVERINFO | CVAR_LATCH);
	// Lets make sure the streak doesn't get abused.
//	if (DHMG_Start->value > 20)
//		DHMG_Start->value = 20;

//	if (DHMG_Start->value < 10)
//		DHMG_Start->value = 10;
	// END
